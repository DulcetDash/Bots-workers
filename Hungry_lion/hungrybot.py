...
def launchBot():
    try:
        ...
        main_categories = categories_page.find_all('figcaption', {'class':'widget-image-caption wp-caption-text'})
        ...
        for i, element in enumerate(main_categories):
            ...
            if category not in ['DEALS']:
                ...
                display_log(Fore.BLUE, '-> subcategories found {}'.format(len(main_subcategories)))
                ...
                for j, subcategory in enumerate(main_subcategories):
                    if subcategory.has_attr('data-jltma-wrapper-link'):
                        data_regulator = json.loads(subcategory['data-jltma-wrapper-link'])
                        ...
                        if data_regulator['is_external'] is not 'on':
                            if subcategory.find('figcaption', {'class':'widget-image-caption wp-caption-text'}) is not None and str(subcategory.find('figcaption', {'class':'widget-image-caption wp-caption-text'}).get_text()).strip().upper() not in 'DAILY SPECIALS':
                                subcategory_name = str(subcategory.find('figcaption', {'class':'widget-image-caption wp-caption-text'}).get_text()).strip()
                                #Get the next link url
                                products_url = data_regulator['url']

                                #? Open the products page
                                products_megadata = getHTMLSOUPEDDocument(products_url)

                                products_parent  = products_megadata.find_all('section')[10].parent
                                ...
                                if products_parent is not None and products_parent.find('div', {'class':'elementor-row'}) is not None:
                                    ...
                                    if products_parent_l1 is not None:
                                        ...
                                        products_parent_l2 = products_parent_l1.find_all('section', {'class':'has_ma_el_bg_slider'})
                                        # print(len(products_parent_l2))

                                        #? Get all the products
                                        for j, product_l3 in enumerate(products_parent_l2):
                                            #Get the products in a row
                                            isolated_products = product_l3.find('div', {'class':'elementor-row'}).find_all('div', {'class':'has_ma_el_bg_slider'})
                                            
                                            for k, product in enumerate(isolated_products):
                                                display_log(Fore.LIGHTRED_EX, '[*] {}% - {}'.format(round(i*100/len(main_categories)),category))
                                                display_log(Fore.CYAN, '{}% - {}'.format(round(j*100/len(main_subcategories)),subcategory_name))
                                                
                                                product = product.find('div', {'class':'elementor-widget-wrap'}).find_all('div', {'class':'elementor-element'})
                                                if len(product) > 0:
                                                    try:
                                                        #Get the name, price and picture
                                                        # print(product)
                                                        product_name = str(product[1].find('h4', {'class':'elementor-heading-title'}).get_text()).strip()
                                                        product_price = str(product[2].find('h4', {'class':'elementor-heading-title'}).get_text()).strip()
                                                        product_image = str(product[0].find('img')['src']).strip()
                                                        product_description = str(product[0].find('p', {'class':'jltma-image-hover-desc'}).get_text()).strip()
