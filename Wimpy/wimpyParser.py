...
shop_fp = 'wimpy32309434jf33328837887322322'
_SHOP_NAME_ = 'WIMPY'
...
def process_product(product):
    ...
    if len(product_name.strip()) == 0:
        print('Product name not found, item skipped')
        return False

    currency, price = Utility.extract_currency_and_price(priceString)

    root_url = 'https://namibia.wimpy.africa'
    ...
    category = product_link.split('/')
    category = str(category[len(category) - 2]).strip().upper()
