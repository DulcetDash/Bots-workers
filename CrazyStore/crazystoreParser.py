...
data = Utility.csv_to_json('Catalogue-Nov-8-2023-Crazystore.csv')

shop_fp = 'crazystore839283237320032322322'
_SHOP_NAME_ = 'CRAZY STORE'
main_url = 'https://www.crazystore.co.za'
...
def process_product(product):
    ...
    if len(priceString) > 1:
        priceString = str(priceString[1]).strip()
    else:
        priceString = str(priceString[0]).strip()
    ...
    product_link = str(product['web-scraper-start-url']).strip()
    category = 'ALL'
